...
fsize = 20
####################################################################
#python (*.py) splitdos.dat
####################################################################

#####SETTING#####
vb_ran= 0.45		#fitting range
cb_ran= 0.45
vb_expinf = -0.3	#expected position of the inflection point
cb_expinf = 0.3
...
cbR = int(findroot(avg.data[0][idxlist],yp[1],cb_expinf-0.2,cb_expinf+0.2)/step)
xini = np.linspace(avg.data[0][zero_idx + cbR]-0.1,avg.data[0][zero_idx + cbR]+0.1,inipts)
gau_cb = yp
...
vbR = int(findroot(avg.data[0][idxlist],yp[1],vb_expinf-0.2,vb_expinf+0.2)/step)
xini = np.linspace(avg.data[0][zero_idx+vbR]-0.1,avg.data[0][zero_idx+vbR]+0.1,inipts)
gau_vb = yp
...
prop_fit={'linestyle':'--','lw':2}
axes=plt.gca()
axes.set_xlim(xlim)
axes.set_ylim(ylim)
vb,  = ax.plot(avg.data[0],avg.data[1]**0.5,color='red',lw=2)
vb_fit = ax.plot(vbx, (vbpara[1]+(vbx)*vbpara[0])**0.5,':r',**prop_fit)
cb,  = ax.plot(avg.data[0],avg.data[2]**0.5,color='blue',lw=2)
cb_fit = ax.plot(cbx, (cbpara[1]+(cbx)*cbpara[0])**0.5,':b',**prop_fit)
#ax.axvspan(vfitini,vfitini-vb_ran, alpha=0.2, color='red')
#ax.axvspan(cfitini,cfitini+cb_ran, alpha=0.2, color='blue')
ax.tick_params(direction='out',labelsize=fsize)
ax.tick_params(pad=15)
afont = {'fontname':'Arial', 'fontsize':fsize}
prop_leg={'loc':2,'fontsize':fsize,'frameon':False}
plt.ylabel("DOS (states/eV)",**afont)
plt.xlabel("Fermi energy (eV)",**afont)
plt.tight_layout()
...
ax2.axvspan(vb_expinf-0.2,vb_expinf+0.2, alpha=0.2, color='red')
ax2.axvspan(cb_expinf-0.2,cb_expinf+0.2, alpha=0.2, color='blue')
plt.tight_layout()
plt.show()
